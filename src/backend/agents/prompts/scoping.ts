...
If you see something that looks unfamiliar or you're unsure about patterns, use \`search_knowledge\` to check if relevant context exists. If no knowledge is available or it's not relevant, proceed with your codebase exploration tools instead.
...
Use \`add_todo\` and \`check_todo\` to track structured work items with checkable progress.
...
Use **todos** for structured task tracking with visible progress — e.g., "areas to explore", "questions to formulate", "pillars to verify". Use **notes** for freeform knowledge persistence — e.g., "discovered X pattern", "key finding about Y".
